...
for regName in FS MSMSulc MSMAll; do
    if [[ ! -e "$AtlasSpaceFolder/$Subject.StrainJ_$regName.$mesh.dscalar.nii" && -e "$AtlasSpaceFolder/$Subject.ArealDistortion_$regName.$mesh.dscalar.nii" ]]; then
        echo "... using ArealDistortion_${regName} map in place of StrainJ_${regName}" > /dev/null 2>&1
...
if [[ -e "$acpc2MNILinear" ]]; then
    ...
    then
        ...
        wb_command -volume-affine-resample \
            ...
            -flirt "$volumeIn" "$volumeRef" > /dev/null 2>&1
...
wb_command -volume-math "ln(x)/ln(2)" "$jacobianLog2" -var x "$jacobian" > /dev/null 2>&1
...
for hemi in L R; do 
    ...
    wb_command -volume-to-surface-mapping $jacobianLog2 "$surf" \
        "$OutputSceneFolder/$Subject.$mapName.$hemi.$mesh.func.gii" -trilinear > /dev/null 2>&1
done
...
wb_command -cifti-create-dense-scalar "$OutputSceneFolder/$Subject.$mapName.$mesh.dscalar.nii" \
  ...
  -right-metric "$OutputSceneFolder/$Subject.$mapName.R.$mesh.func.gii" > /dev/null 2>&1
wb_command -set-map-names "$OutputSceneFolder/$Subject.$mapName.$mesh.dscalar.nii" -map 1 "${Subject}_$mapName" > /dev/null 2>&1
...
for ((ind = 1; ind <= numScenes; ind++)); do
    wb_command -show-scene "$OutputSceneFolder/$sceneFile" $ind "${pngDir}/${sceneFile}${ind}.png" 100 100 -use-window-size > /dev/null 2>&1
done
